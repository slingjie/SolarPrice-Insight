...
## 2026-02-08 省份电价详情页视图重构 - 年度趋势图与数据表
- **需求背景**：用户希望更直观地查看年度电价趋势，而非分散的月度卡片。
- **涉及组件**：`Dashboard.tsx`, `package.json` (引入 recharts)。
- **改进点**：
    - **视图重构**：移除了月度卡片网格，替换为年度电价趋势图（LineChart）和详细数据表（Table）。
    - **交互优化**：移除了“执行月份”筛选，实现了按年度查看尖峰平谷深趋势。
    - **筛选保留**：保留了“用电分类”和“电压等级”筛选，确保数据的准确对比。
    - **数据展示优化**：处理了电价数据中的 `0` 值，将其视为数据缺失（`null`），确保在趋势图中显示为断点而非下坠到 0，且不自动连接断点（`connectNulls: false`）。
- **验证情况**：趋势图清晰显示各时段价格波动，缺失数据正确显示为断线，数据表格展示完整详细，筛选功能正常联动。
