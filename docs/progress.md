...
## 2026-01-03 Phase 2: 数据管理中心 (Admin Module) 上线
- **需求背景**：随着电价数据、时段配置及综合结果的增多，原有的分散式管理（各页面独立管理）已无法满足需求。需构建统一的后台管理界面，并提供数据备份保障。
- **涉及组件**：`AdminModule`, `AdminLayout`, `TariffsManager`, `TimeConfigsManager`, `ResultsManager`, `DataImportExport`, `BackupRestore`, `Settings`, `App`.
- **核心功能落地**：
    1.  **统一管理门户**：
        - 新增 `/admin` 路由及 `AdminLayout` 布局，提供侧边栏导航。
        - 实现了 Dashboard 数据概览，实时统计各模块数据量。
    2.  **分模块管理**：
        - **电价数据 (Tariffs)**：支持分页列表、全局搜索、字段排序。新增“去重查询”功能（按价格或完全匹配去重）。支持 CRUD 及批量删除。
        - **时段配置 (TimeConfigs)**：集成 Matrix 视图，支持按省份管理配置。
        - **综合结果 (Results)**：提供计算结果的查询与删除管理，支持结果回溯。
    3.  **数据安全与迁移**：
        - **JSON 导入导出**：支持标准 JSON 格式的数据迁移。
        - **全量备份恢复**：提供一键备份所有数据（Metadata + Data），支持从备份文件全量恢复（Upsert 模式）。
    4.  **架构调整**：
        - 移除了散落在 `ManualEntry` 和 `Settings` 中的旧版管理入口。
        - `Settings` 页面新增“进入数据管理中心”大按钮，作为唯一管理入口。
        - **(New!)** 支持手动新增电价数据 & 新建时段配置。
        - **(New!)** 电价数据管理页支持“深谷”电价配置。
        - **(New!)** 实现了全量数据备份与恢复（JSON 格式）。
        - **(New!)** 实现了针对 Tariffs、TimeConfigs、Results 的分类导出。
        - **(修复)** 解决了 Chrome 浏览器下下载请求被静默拦截的问题。
        - **(修复)** 解决了文件选择弹窗在某些浏览器环境下的点击唤起问题。
- **验证情况**：
    - 导出功能在正式浏览器（Chrome/Safari）中已验证正常。
    - 导入功能（数据合并与覆盖逻辑）逻辑验证通过。
    - 不同浏览器间的数据存储独立性已在 UI 中添加详细说明。
    - **(New!)** 操作日志功能已实现，支持记录新增/编辑/删除/批量删除/恢复等操作。
    - **修复**: 解决了 RxDB Schema 中使用 `collection` 保留字导致的 SC17 错误，修复了数据无法保存的问题。
    - **优化**: 改进了 Chrome 下文件选择器的唤起逻辑，优先使用原生的 `showPicker()` 提高兼容性。
