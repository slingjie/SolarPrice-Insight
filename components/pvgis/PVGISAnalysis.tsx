...
interface PVGISAnalysisProps {
    onBack?: () => void;
}

export const PVGISAnalysis: React.FC<PVGISAnalysisProps> = ({ onBack }) => {
    ...
    return (
        <div className="h-full flex flex-col md:flex-row gap-6 p-6 animate-in fade-in bg-slate-50 relative">
            {/* Portal Header - Only visible in Portal Mode (when sidebar is hidden) */}
            ...
            {onBack && (
                <div className="fixed top-0 left-0 p-6 z-50 flex items-center gap-4 group">
                    <button
                        onClick={onBack}
                        className="bg-white px-3 py-2 rounded-xl shadow-xl shadow-slate-200/50 border border-slate-100 text-slate-600 hover:text-orange-600 transition-all flex items-center gap-3"
                    >
                        <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-colors">
                            <ArrowLeft size={18} />
                        </div>
                        <span className="font-bold">返回模块门户</span>
                    </button>
                    <div className="flex items-center gap-2 opacity-40 ml-2">
                        <Sun className="text-orange-500" size={20} />
                        <span className="font-bold text-slate-700">SolarPrice</span>
                    </div>
                </div>
            )}
